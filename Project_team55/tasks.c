...
static void GateState_Set(GateState_t s) {
    ...
    /* TC-20: mutex acquired – no other task can read/write gateState now */
    vprintString("[TC-20][Mutex] ACQUIRED -> state WRITE by GateState_Set\n");
    gateState = s;
    vprintString("[TC-20][Mutex] RELEASED -> state WRITE done\n");
...
static bool GateState_CAS(GateState_t expected, GateState_t next) {
    bool ok = false;
    xSemaphoreTake(xGateStateMutex, portMAX_DELAY);
    /* TC-20: mutex acquired – atomic compare-and-swap protects state transition */
    vprintString("[TC-20][Mutex] ACQUIRED -> CAS transition by GateState_CAS\n");
    if (gateState == expected) { gateState = next; ok = true; }
    vprintString("[TC-20][Mutex] RELEASED -> CAS done\n");
    xSemaphoreGive(xGateStateMutex);
    return ok;
}
...
void vInputTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (evt != EVT_NONE) {
            ...
            /* TC-19: verify queue delivers events without loss under rapid input */
            if (xQueueSend(xInputQueue, &qEvt, 0) == pdTRUE) {
                vprintString("[TC-19][Queue] Event SENT  – queue OK\n");
            } else {
                vprintString("[TC-19][Queue] Event DROPPED – queue FULL (loss detected!)\n");
            }
        }
...
void vGateControlTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xSemaphoreTake(xLimitOpenSem, 0) == pdTRUE) {
            /* TC-21: Limit-Open semaphore received – gate stops at fully-open position */
            vprintString("[TC-21][Semaphore] Limit-Open semaphore TAKEN – gate stopped at IDLE_OPEN\n");
        ...
        if (xSemaphoreTake(xLimitCloseSem, 0) == pdTRUE) {
            /* TC-21: Limit-Close semaphore received – gate stops at fully-closed position */
            vprintString("[TC-21][Semaphore] Limit-Close semaphore TAKEN – gate stopped at IDLE_CLOSED\n");
        ...
        /* TC-19: log every event received from the queue by GateCtrl */
        vprintString("[TC-19][Queue] Event RECEIVED by GateCtrl – queue delivery confirmed\n");
 ...
 *
 * Reads gateState every 50 ms and drives the two directional LEDs wired to
 * Port A (PA2 = Green "Opening" LED, PA3 = Red "Closing" LED).
 * These LEDs indicate gate movement direction only; both are off whenever
 * the gate is idle, stopped, or not actively driven.
 * This task never touches Port F (the onboard RGB LED).
 *
 * | State                | PA2 (Green LED) | PA3 (Red LED) |
 * |----------------------|-----------------|---------------|
 * | GATE_IDLE_CLOSED     |        0        |       0       |
 * | GATE_IDLE_OPEN       |        0        |       0       |
 * | GATE_OPENING         |        1        |       0       |
 * | GATE_CLOSING         |        0        |       1       |
 * | GATE_STOPPED_MIDWAY  |        0        |       0       |
 * | GATE_REVERSING       |        1        |       0       |
 * ═══════════════════════════════════════════════════════════════════════════*/
void vLEDControlTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        /* Drive PA2/PA3 direction LEDs to reflect gate movement */
        switch (state) {
        case GATE_OPENING:
            Motor_SetDir(true, false);   /* PA2 HIGH (Green ON),  PA3 LOW  (Red OFF) */
            break;

        case GATE_CLOSING:
            Motor_SetDir(false, true);   /* PA2 LOW  (Green OFF), PA3 HIGH (Red ON)  */
            break;
        ...
        default:
            /* IDLE_CLOSED, IDLE_OPEN, STOPPED_MIDWAY – both direction LEDs off */
            Motor_Stop();
            break;
        }
    }
...
/* ═══════════════════════════════════════════════════════════════════════════
 * TASK 5 – Status_Task  (Lowest Priority = 1)
 *
 * Reads gateState every 50 ms and drives the Tiva-C on-board RGB LED on
 * Port F (PF1 = Red, PF2 = Blue, PF3 = Green) to display the full system
 * status across all six FSM states.
 * This task never touches Port A (the directional LEDs).
 *
 * | State                | Colour  | PF3 (Green) | PF2 (Blue) | PF1 (Red) |
 * |----------------------|---------|-------------|------------|----------|
 * | GATE_IDLE_CLOSED     | Blue    |      0      |     1      |     0     |
 * | GATE_IDLE_OPEN       | Cyan    |      1      |     1      |     0     |
 * | GATE_OPENING         | Green   |      1      |     0      |     0     |
 * | GATE_CLOSING         | Red     |      0      |     0      |     1     |
 * | GATE_STOPPED_MIDWAY  | Yellow  |      1      |     0      |     1     |
 * | GATE_REVERSING       | Magenta |      0      |     1      |     1     |
 * ═══════════════════════════════════════════════════════════════════════════*/
void vStatusTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        /* Update onboard RGB LED (Port F) to reflect current system state */
        switch (state) {
        case GATE_IDLE_CLOSED:
            LED_AllOff();
            LED_Set(BLUE_LED, true);                    /* Blue    – fully closed, at rest    */
            break;

        case GATE_IDLE_OPEN:
            LED_AllOff();
            LED_Set(GREEN_LED | BLUE_LED, true);        /* Cyan    – fully open, at rest      */
            break;

        case GATE_OPENING:
            LED_AllOff();
            LED_Set(GREEN_LED, true);                   /* Green   – gate moving open         */
            break;

        case GATE_CLOSING:
            LED_AllOff();
            LED_Set(RED_LED, true);                     /* Red     – gate moving closed       */
            break;

        case GATE_STOPPED_MIDWAY:
            LED_AllOff();
            LED_Set(RED_LED | GREEN_LED, true);         /* Yellow  – stopped mid-travel       */
            break;

        case GATE_REVERSING:
            LED_AllOff();
            LED_Set(RED_LED | BLUE_LED, true);          /* Magenta – reversing after obstacle */
            break;
